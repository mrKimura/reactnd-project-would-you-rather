...
class Dashboard extends Component {
  ...
  handleListChange = event => {
    const { id: activeQuestions } = event.target
    const { activeQuestions: stateActiveQuestions } = this.state
    if (activeQuestions !== stateActiveQuestions) {
      this.setState({
        activeQuestions,
      })
    }
    ...
    return (
      <div className="dashboard container">
        <div className="dashboard__header dashboard-header">
          <a
            className={`dashboard-header__item ${
              activeQuestions === 'UnAnswered' ? 'dashboard-header__item--active' : ''
            }`}
            id="UnAnswered"
            onClick={this.handleListChange}
            onKeyDown={this.handleListChange}
          >
            Unanswered Questions
          </a>
          <a
            className={`dashboard-header__item ${
              activeQuestions === 'Answered' ? 'dashboard-header__item--active' : ''
            }`}
            id="Answered"
            onClick={this.handleListChange}
            onKeyDown={this.handleListChange}
        ...
        <div className="dashboard__questions">
          {activeQuestions === 'UnAnswered'
            ? unAnsweredQuestions.map(q => <DashBoardItem key={q} id={q} />)
            : answeredQuestions.map(q => <DashBoardItem key={q} id={q} />)}
        </div>
      </div>
    )
  }
...
const getQuestionList = (allQuestions, authedUser) => {
  ...
  Object.keys(allQuestions).forEach(
    questionIndex =>
      [
